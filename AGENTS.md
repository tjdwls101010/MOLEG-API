...
This project builds the MOLEG-API layer that a future legislative-expert skill will use to inspect Korean law from `law.go.kr`. It owns the legal-source interface after a bill reaches the 법제처 side: current statutes, effective text, articles, delegated rules, administrative rules, official interpretations, cases, constitutional decisions, legal terms, and related-law/query expansion.
...
- `congress-db` direct SQL owns National Assembly facts such as bills, votes, minutes, and promulgation bridge fields.
- MOLEG-API owns enacted/current legal sources and their relationships.
- WebSearch owns latest social context, statistics, news, and government announcements outside MOLEG's corpus.
...
- Law-name search can be ambiguous, so do not silently take the first plausible result.
- Statute text alone can miss enforcement decrees, enforcement rules, notices, and administrative rules.
- MOLEG interpretations, ministry first-instance interpretations, Supreme Court cases, and Constitutional Court decisions have different authority and must remain distinguishable.
- Secrets such as `MOLEG_OC` stay in environment variables and never in committed files.

Keep durable detail in the normal project documents: `CONTEXT.md` for language, `docs/design/PRD.md` for scope and user stories, `docs/design/MOLEG-API-AUDIT.md` for catalog classification, `docs/design/DECISIONS.md` for hard-to-reverse decisions, and `docs/SKILL-INTEGRATION.md` for how the future skill should use this layer.
...
1. **Branch.** Name it after the issue (e.g. `12-checkout-discount`).
...
7. **Mark ready for review.** Drop "draft", add the closing summary. The PM reviews per-slice; merge slice by slice.
