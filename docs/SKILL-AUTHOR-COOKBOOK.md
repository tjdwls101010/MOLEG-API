...
Use effective-date basis for current-force questions. Use promulgation-date basis only when reconstructing the source bridge or historical promulgation context. Resolving a promulgation bridge or loading current text does not prove what the enacted bill changed; use `trace_law_history()` or `compare_law_versions()` before describing amendment deltas. For definitions, application targets, exceptions, or requirements, use normalized `ArticleText.text` / `AdministrativeRuleArticleText.text`; MOLEG-API preserves nested 항, 호, and 목 there, so do not summarize from `조문제목` or top-level `조문내용` alone. Check article status fields before treating loaded article text as operative: `is_deleted=True` or `revision_type="삭제"` means the loaded article is deleted source state, not a current duty or permission. For 시행일, 적용례, or 경과조치 questions, load full law text and cite `LawText.supplementary_provisions` separately from main articles; do not infer transition scope from law-level `effective_date` metadata alone. For 구법 or 폐지법 questions, use an explicit historical `as_of` date and disclose that date before citing text; current effective no-result is not proof that the historical source never existed. For bundle-based current-force checks, pass an explicit `as_of` reference date and treat `not_effective_as_of` as "promulgated/source-loadable but not yet effective." For bundle-loaded interpretation/case/Constitutional Court detail, treat `authority_article_mismatch`, `authority_article_unverified`, `authority_article_partial_match`, and `authority_temporal_mismatch` gaps as blockers for unsupported current target-article authority claims until scoped follow-up searches, structured article references, or history/as-of checks support the target article. Treat `search_administrative_rules(issued_on=...)` as a 발령일자 filter, not an as-of effective-date lookup. For selected administrative rules, compare `AdministrativeRuleText.identity.effective_date` to the same reference date before describing the rule as current operational criteria, and inspect `AdministrativeRuleText.supplementary_provisions` when administrative-rule 부칙 matters.
...
- `gaps` tells the skill when WebSearch, human review, or more authority-source loading is needed.

Do not treat `LegalContextBundle` as a legal conclusion. It is a context-loading result and planning guide.
Likewise, `expand_legal_query()` output is not citation material; use its follow-ups to load selected source text before making legal-basis claims.
...
Read it like a staged bundle: loaded laws, `law_structures`, and delegations are available first; administrative rules, annex/forms, interpretations, cases, and Constitutional Court decisions may remain candidates or deferred lookups. If `not_effective_as_of` appears, disclose that the affected statute is outside current-force scope on the reference date. After loading selected administrative-rule detail, compare its `effective_date` to `bundle.request.as_of` before treating it as current operational criteria. If the statute set is uncertain, run `search_laws()` or `expand_legal_query()` first and surface ambiguity instead of using this helper as a discovery engine.
...
`get_annex_form_body()` uses law.go.kr text-export surfaces for law and administrative-rule annex/forms. Structured rows are conservative convenience data; the plain `text` remains the source fallback for irregular or low-confidence tables. If `structured_data.parsing_confidence` is low, empty rows do not prove that no criteria or thresholds exist. Direct HWP/PDF parsing is intentionally outside this interface.
...
The `detc` source does not expose structured doctrine/category fields. Doctrine terms such as `과잉금지원칙` or `평등원칙` are free-text search terms, not source-backed filters. Cite loaded decision detail and `reviewed_articles`, but do not claim exhaustive doctrine coverage or "no constitutional risk" from free-text search alone.
...
- Do not reuse a MOLEG legal citation as the source for latest statistics or news; cite WebSearch results separately.
- Do not reuse a main-article citation as the source for 부칙 시행일, 적용례, or 경과조치; cite `supplementary_provisions` separately.
