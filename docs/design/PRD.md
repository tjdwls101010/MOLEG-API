...
- `MolegApi.load_legal_context_bundle()` composes the task-level interfaces into a staged bundle for Claude, with loaded statute/article/delegation context, bounded administrative-rule, annex/form, interpretation, and judicial candidates, explicit deferred follow-up detail loading, ambiguity records, and structured WebSearch gaps.
