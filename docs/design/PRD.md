...
- `MolegApi.load_delegated_criteria()` starts from a known statute anchor and reuses the institutional-system bundle shape, but additionally loads bounded selected administrative-rule bodies and annex/form bodies into `LoadedContext`. It is for operational-criteria questions where candidate metadata is not enough; administrative-rule detail goes through article-context semantics so deleted/moved article markers stay as gaps/source notes and only current destination articles are promoted as loaded operational criteria. Detail-load failures remain gaps plus deferred lookups, loaded administrative-rule source law/article references are checked against the target statute/article with `delegated_criteria_source_mismatch` or `delegated_criteria_source_unverified` gaps, loaded annex/form source references are checked against the target statute or a source-verified administrative rule with `delegated_criteria_annex_source_mismatch` or `delegated_criteria_annex_source_unverified` gaps, and `as_of` still emits `not_effective_as_of` for future-effective loaded administrative rules.
