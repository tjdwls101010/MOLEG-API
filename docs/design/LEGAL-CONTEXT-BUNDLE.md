...
7. If eager-loaded interpretation/case/Constitutional Court detail is present but `authority_article_mismatch`, `authority_article_unverified`, `authority_article_partial_match`, or `authority_temporal_mismatch` appears in `gaps`, treat that detail as follow-up search/history context only for the unmatched, unverified, missing, or temporally stale requested articles. Cite the loaded target article itself if useful, then run the recommended scoped authority searches or history/as-of checks before making a current target-article authority claim for those articles.
