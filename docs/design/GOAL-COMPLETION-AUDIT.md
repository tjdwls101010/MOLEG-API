...
- Last deterministic command excluding live tests: `python3 -m pytest -q -m 'not live'` -> `110 passed, 54 deselected`.
- PR #89 deterministic GitHub Actions CI passed after #91: latest checked run `27641400110` ran non-live tests on Python 3.10, 3.11, and 3.12 plus a package wheel/install gate with Node 24 opt-in.
...
| Use GitHub issues/branches/PRs to maintain progress visibility. | Initial-core work was tracked through earlier issue/PR slices. The current consumer-readiness work is tracked by umbrella #49, child issues #50-#68 plus #90/#91, and integration draft PR #89 on `codex/49-consumer-readiness-integration-gate`. | Proven by GitHub state and PR #89. |
| Run deterministic and necessary live gates. | Deterministic tests pass with live tests excluded: `python3 -m pytest -q -m 'not live'` -> `110 passed, 54 deselected`. PR #89 now also has GitHub Actions CI for non-live tests across Python 3.10/3.11/3.12 and a package install gate; latest checked run `27641400110` passed. Live source gates pass separately: `tests/test_live_smoke.py` -> `8 passed, 1 skipped`; `tests/test_live_e2e_scenarios.py` -> `44 passed, 1 skipped`. Fresh congress-db introspection also passes with local `.env.local`. | Proven. |
