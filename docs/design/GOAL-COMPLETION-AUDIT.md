...
- Last deterministic command excluding live tests: `python3 -m pytest -q -m 'not live'` -> `130 passed, 54 deselected`.
...
| Prove the skill invocation seam can be installed and imported outside the repository checkout. | `pyproject.toml` defines the `moleg-api` package; current integration-branch verification built a wheel, confirmed package metadata and `py.typed`, installed into an isolated target directory, imported `moleg_api` from `/tmp`, verified the imported package path came from the isolated install target, verified installed `py.typed`, verified every package-root `__all__` export, and verified recursive serialization with and without raw payloads. | Proven by package build/install check. |
