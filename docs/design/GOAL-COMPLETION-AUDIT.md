...
The MOLEG-API layer is implemented for the initial core goal in `.Seongjin/goal.md` and has passed representative live law.go.kr smoke verification, a 41-scenario legislative live e2e gate, and fresh read-only `congress-db` introspection.
...
- Legislative live e2e passed with local credentials: `.venv/bin/python -m pytest tests/test_live_e2e_scenarios.py -q` -> `41 passed`.
...
- Last deterministic command excluding live tests: `.venv/bin/python -m pytest -q -m 'not live'` -> `48 passed, 50 deselected`.
- Last full command with local `.env` `MOLEG_OC` and local `.env.local` `CONGRESS_DB_READONLY_URL`: `.venv/bin/python -m pytest -q` -> `97 passed, 1 skipped`.
...
| Run dozens of realistic e2e scenarios from a legislative-expert Claude perspective. | `tests/test_live_e2e_scenarios.py` covers 41 statute/article, delegation, administrative-rule, annex/form, official/ministry interpretation, case, constitutional, query-planning, bundle, and congress bridge scenarios through public `MolegApi` methods. Latest run: `41 passed`. | Proven by live e2e gate. |
| Avoid overclaiming perfection. | `docs/design/COMPLETION-RISK-AUDIT.md` records residual risks and follow-up issues for annex bodies and full history; recent bridge lag, Constitutional Court detail coverage, and ministry interpretation live coverage are recorded as mitigated. | Proven by risk audit and GitHub issues. |
...
| Run full tests and necessary live smoke tests. | Deterministic tests pass with live tests excluded: `.venv/bin/python -m pytest -q -m 'not live'` -> `48 passed, 50 deselected`. Full suite with local `.env` `MOLEG_OC` and local `.env.local` `CONGRESS_DB_READONLY_URL` passes: `97 passed, 1 skipped`. Fresh congress-db introspection also passes with local `.env.local`. | Proven. |
