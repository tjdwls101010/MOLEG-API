...
| Implement history/comparison next. | `trace_law_history()` and `compare_law_versions()` exist; tests cover JSON-reachable article history, article-scoped post-change `article_text` snapshots, missing-snapshot fallback behavior, full law-level `lsHistory` HTML list parsing, parse failure on changed HTML shape, live full-history loading, and `oldAndNew` normalization. | Proven by deterministic and live tests. |
